...
void main() {
  ...
  testWidgets('a save just started says nothing yet', (tester) async {
    ...
    // "Played 0m" reads like a bug rather than like a new save.
  ...
  testWidgets('minutes up to an hour', (tester) async {
    await pumpL10n(tester);
    expect(playedLabel(l, 60), 'Played 1m');
    expect(playedLabel(l, 90), 'Played 1m');
    expect(playedLabel(l, 45 * 60), 'Played 45m');
  });

  testWidgets('hours and minutes beyond that', (tester) async {
    await pumpL10n(tester);
    expect(playedLabel(l, 3600), 'Played 1h 0m');
    expect(playedLabel(l, 3600 * 3 + 60 * 24), 'Played 3h 24m');
    // A long career reads in hours, never in days — a manager counts hours.
    expect(playedLabel(l, 3600 * 40), 'Played 40h 0m');
  });

  testWidgets('the seconds are never shown', (tester) async {
    await pumpL10n(tester);
    expect(playedLabel(l, 3600 * 2 + 60 * 14 + 37), 'Played 2h 14m');
  });
}
