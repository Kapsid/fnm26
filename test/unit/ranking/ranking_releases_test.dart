...
void main() {
  test('the ranking is published periodically and announced', () async {
    ...
    final releases =
        await container.read(rankingReleaseRepositoryProvider).all(career.id);
    expect(
      releases.length,
      greaterThan(3),
    ...
    final spread = ranks.reduce((a, b) => a > b ? a : b) -
    ...
    final inbox = await container.read(messageInboxProvider(career.id).future);
    final rankMessages =
        inbox.messages.where((m) => m.category == 'ranking').toList();
    expect(
      rankMessages.length,
      releases.length,
    ...
    final ranking = await container.read(worldRankingProvider(career.id).future);
    ...
    final again = await container.read(messageInboxProvider(career.id).future);
    expect(
    ...
    final history =
        await container.read(rankHistoryProvider(career.id).future);
    expect(
      history.length,
