...
void main() {
  test(
    'the cycle calendar runs in order and the finals step round by round',
    () async {
      ...
      for (var i = 0; i < 400; i++) {
        ...
        if (await comp.hasFinals(career.id)) {
          ...
          for (final r in wcRounds) {
            played += (await comp.fixturesByRound(
              career.id,
              r,
            )).where((f) => f.played).length;
          }
      ...
      expect(
        ...
        reason:
            'the finals should advance one matchday at a time, not resolve '
            'in a single step',
      );
      ...
      final finalDate = await firstDate(
        'FINAL',
      ...
      final wcGroupStart = await firstDate(
        'GROUP',
        CompetitionKind.worldCupFinals,
      );
      ...
      expect(
        ncGroupStart.month,
        9,
        reason: 'Nations Cup groups open in autumn',
      );
      ...
      final quals =
          (await comp.allFixtures(career.id))
              .where(
                (f) => f.round == null,
              ) // qualifying fixtures carry no round
              .map((f) => f.date)
              .toList()
            ..sort();
      expect(
        quals.first.month,
        3,
        reason: 'qualifying opens in the March window',
      );
